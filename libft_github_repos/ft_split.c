...
char	**ft_split(char const *s, char c)
{
	...
	while (is_end(se) == 0)
	{
		// Crea una copia de la subcadena antes de agregarla
...
// Función para liberar la memoria de las cadenas y el arreglo de punteros
void free_split_array(struct SplitArray *split_array)
{
	if (split_array->array)
	{
		int i = 0;
		while (i < split_array->size)
		{
			free(split_array->array[i]);  // Libera cada subcadena
			i++;
		}
		free(split_array->array);  // Libera el arreglo de punteros
	}
}
